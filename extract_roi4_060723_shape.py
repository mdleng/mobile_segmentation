...
def checkFinger(image,XY):
    ...
    for ct in range(XY.shape[0]):
        ...
        if x<0 or x>image.shape[1] or y<0 or y>image.shape[0]:
            ...
            if image[y,x,0]==0:
                isgood=False
                print('false cond 2',y,x,image[y,x,0])
               # cv2.imwrite('DEBUGpinky.jpg',image)
    return isgood
...
def get_shape_features_palm(XY_wrist_full,XY_thumb_full, XY_index_full,XY_middle_full,XY_ring_full,XY_pinky_full):
    ...
    ### feat 1 wrist-thumb
    pTop=XY_thumb_full[0,:]    
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist1=np.sqrt(dx*dx + dy*dy)/dist_ref
    
    ### feat 2 wrist-index
    pTop=XY_index_full[0,:]    
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist2=np.sqrt(dx*dx + dy*dy)/dist_ref
    
    ### feat 3 wrist-ring
    pTop=XY_ring_full[0,:]    
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist3=np.sqrt(dx*dx + dy*dy)/dist_ref
    
     ### feat 4 wrist-pinky
    pTop=XY_pinky_full[0,:]    
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist4=np.sqrt(dx*dx + dy*dy)/dist_ref
    ...
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist5=np.sqrt(dx*dx + dy*dy)/dist_ref
    ...
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist6=np.sqrt(dx*dx + dy*dy)/dist_ref
    ...
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist7=np.sqrt(dx*dx + dy*dy)/dist_ref
    ...
    dx=pBottom[0]-pTop[0]
    dy=pBottom[1]-pTop[1]
    dist8=np.sqrt(dx*dx + dy*dy)/dist_ref
    
    return np.array([dist1,dist2,dist3,dist4,dist5,dist6,dist7,dist8])
...
def get_shape_features(roi,dist_ref):
    ...
    roi_bin=cv2.erode(roi_bin,np.ones((kernelsize,kernelsize),dtype='uint8'))
    roi_bin=cv2.dilate(roi_bin,np.ones((kernelsize,kernelsize),dtype='uint8'))
    
    [r,c]=np.where(roi_bin>0)
    ...
    roi=roi[y1_crop:y2_crop,x1_crop:x2_crop]
...
if __name__ == '__main__':
    ...
    path_shape=os.path.join(path_data,'shape')
    if os.path.exists(path_shape) and args.clear=='True':
            shutil.rmtree(path_shape)
    if not os.path.exists(path_shape):
            os.mkdir(path_shape)

    unlabel_paths=['fingersfront','palmfront','fingersback','palmback']
    for p in unlabel_paths:
        p_full=os.path.join(path_shape,p)
    ...
    for id_img in range(len(images)):
     ...
     try:
        ...
        if os.path.exists(filename_img_pad):
            ...
            kernel=np.zeros((11,11),dtype='uint8')
            mask_bin=cv2.erode(mask_bin,kernel)
            mask_bin=cv2.dilate(mask_bin,kernel)
            
            distT = cv2.distanceTransform(mask_bin, cv2.DIST_L2, 3)
            distT =(distT /np.max(distT))*255
            cv2.imwrite('DEBUG_distT.jpg',distT)
            ...
            isgood=True#checkFinger(img,XY_index_full)
            #print('isgood',isgood)
            if isgood:
                ...
                features_shape_palm=get_shape_features_palm(XY_wrist_full,XY_thumb_full, XY_index_full,XY_middle_full,XY_ring_full,XY_pinky_full)
                roi_palm_final=cv2.resize(roi_palm_final,(512,512))
                
                ### write palm roi e features
                if hand_type[1]=='Palm':
                    path_out_palm=os.path.join(path_shape,'palmfront')
                else:
                    path_out_palm=os.path.join(path_shape,'palmback')
                ...
                for finger_type in sorted(list(labels_fingers.keys())):
                    ...
                    if finger_type=='pinky':
                        ...
                        print('pinky',isgoodPinky,XY_pinky_full)
                    ...
                    if save_finger:
                        ...
                        if hand_type[1]=='Palm':
                            path_out_finger=os.path.join(path_shape,'fingersfront')
                        else:
                            path_out_finger=os.path.join(path_shape,'fingersback')
