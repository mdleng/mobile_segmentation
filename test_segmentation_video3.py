...
def refine_palm(image,mask_palm,L,refiner):
  
  [y,x]=np.where(mask_palm>0)
  k=50
  y1=max(0,int(np.min(y))-k)
  y2=min(image.shape[0],int(np.max(y))+k)
  x1=max(0,int(np.min(x))-k)
  x2=min(image.shape[1],int(np.max(x))+k)
  image_roi=image[y1:y2,x1:x2,:]
  mask_palm_roi=mask_palm[y1:y2,x1:x2]

  output = refiner.refine(image_roi, mask_palm_roi, fast=True, L=L)
  
  mask_palm_new=np.zeros_like(mask_palm)                 
  mask_palm_new[y1:y2,x1:x2]=output
  
  
  return mask_palm_new
...
def finger_segmentation(img,index,middle,pinky,thumb,ring,wrist,refiner,L_refiner ):
    ...
    output = refiner.refine(image_flip, mask, fast=True, L=300)
    dmin[output==0]=0

    palm=np.zeros_like(output)
    palm[dmin==60]=255
    image_flip_palm=image_flip.copy()
    r=image_flip_palm[:,:,2]
    g=image_flip_palm[:,:,1]
    b=image_flip_palm[:,:,0]
    b[mask_palm_border>0]=255
    image_flip_palm[:,:,2]=r
    image_flip_palm[:,:,1]=g
    image_flip_palm[:,:,0]=b
    palm_refined=refine_palm(image_flip_palm,palm,L=100,refiner=refiner)
    
    output[palm_refined>100]=255
...
def get_mp_mask(image,hands):
    ...
    if not results.multi_hand_world_landmarks:
      ...
      return np.zeros((image.shape[0], image.shape[0]),dtype='uint8'),None
...
if __name__ == '__main__':
    ...
    parser.add_argument("--clear_dataset", type=bool,default=False)
    ...
    if os.path.exists(path_out) and args.clear_dataset:
    ...
    path_out_images=os.path.join(path_out,'images')
    path_out_masks=os.path.join(path_out,'masks')
    ...
    for filename in files:
            ...
            if args.nframes==0:
              ...
              nframes = min( args.nframes,int(cap.get(cv2.CAP_PROP_FRAME_COUNT))-start_frame)

            frame_width=int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            frame_height=int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

            frame_width_orig=frame_width
            frame_height_orig=frame_height

            print('frame rate' +str(fps))
            print('frame numbers' +str(nframes))
            print('frame_width' +str(frame_width))
            print('frame_height' +str(frame_height))
            print('fCAP_PROP_FRAME_COUNT', int(cap.get(cv2.CAP_PROP_FRAME_COUNT)))
            
            if size_img==0:
              scale_inp=1
            else:
              scale_inp= size_img/max(frame_width,frame_height)
            
            frame_width=int(frame_width*scale_inp)
            frame_height=int(frame_height*scale_inp)
            
            
            #scale_out=1
            frame_width_out=int(scale_out*frame_width_orig)
            frame_height_out=int(scale_out*frame_height_orig)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')

            basename=os.path.basename(filename)
            filename_out=os.path.join(path_out,basename)

            output_video = cv2.VideoWriter(filename_out,fourcc, int(fps), (frame_width_out,frame_height_out))
            ...
            for ct in range(nframes):
                ...
                if ret:
                    ...
                    if results is not None:
                          ...
                          frame_mask=np.zeros((frame.shape[0],frame.shape[1]),dtype='uint8')
                          frame_view3=frame.copy()
                          ...
                          cv2.drawContours(frame_mask, contours1, -1, (255), -1)
                          cv2.drawContours(frame_mask, contours2, -1, (255), -1)
                          cv2.drawContours(frame_mask, contours3, -1, (255), -1)
                          cv2.drawContours(frame_mask, contours4, -1, (255), -1)
                          cv2.drawContours(frame_mask, contours5, -1, (255), -1)
                          cv2.drawContours(frame_mask, contours_palm, -1, (255), -1)
